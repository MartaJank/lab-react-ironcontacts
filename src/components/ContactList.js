...
class ContactList extends Component {
  ...
  addRandomContact = () => {
    ...
    let inList = true;
    let randomContact;
    const firstFive = this.state.Contacts;
    while (inList) {
      randomContact = contacts[Math.floor(Math.random() * contacts.length)];
      inList = false;
      for (let i = 0; i < firstFive.length; i++) {
        if (firstFive[i].name === randomContact.name) {
          inList = true;
        }
      }
    }
    ...
    /* this.setState((state) => ({
            Contacts: state.Contacts.includes(randomContact) ? null : state.Contacts.concat(randomContact)
        })) */
  };

  sortByName = () => {
    const sortedxName = this.state.Contacts;
    sortedxName.sort((a, b) => a.name.localeCompare(b.name));
    this.setState(() => ({
  ...
  render() {
    return (
      <div>
        <h1>IronContacts</h1>
        <div class="main-btns">
          <button class="btn" onClick={this.addRandomContact}>
            Add Random Contact
          </button>
          <button class="btn" onClick={this.sortByName}>
            Sort by name
          </button>
          <button class="btn" onClick={this.sortByPopularity}>
            Sort by popularity
          </button>
        </div>
        <table>
          ...
          {this.state.Contacts.map((oneContact, index) => {
            return (
              <tr>
                ...
                <td>
                  <button class="btn" onClick={() => this.deleteContact(index)}>
                    Delete
                  </button>
                </td>
              </tr>
